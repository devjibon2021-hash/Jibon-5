...
/**
 * Triggers professional, print-styled PDF generation for the current report.
 * Works seamlessly across modern browsers, handling A4 margins, clean typography,
 * page-break controls, and official verification blocks.
 */
export function exportReportToPDF(options: {
  ...
  // Try standard print which triggers browser Save as PDF
  ...
  // Create an isolated hidden iframe for printing to avoid printing outer navigation and headers
  ...
  // Inject styles and clone printable area
  doc.open();
  doc.write(`
    <!DOCTYPE html>
      ...
      <body>
        <div class="report-container">
          ...
          <div class="avoid-break signatures">
            <div>
              <div class="sig-line">${isBn ? 'হিসাব প্রস্তুতকারক' : 'Prepared By'}</div>
            </div>
            <div>
              <div class="sig-line">${isBn ? 'যাচাইকারী' : 'Verified By'}</div>
            </div>
            <div>
              <div class="sig-line">${isBn ? 'পারিবারিক প্রধান / অভিভাবক' : 'Head of Family'}</div>
            </div>
          </div>
